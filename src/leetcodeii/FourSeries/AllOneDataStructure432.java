...
public class AllOneDataStructure432 {
     ...
     * TODO: optimize
     *
     * core: very basic idea.
     *
     * 1. link values using linked list, e.g. 4<->3<->2<->1<->
     *     if each value has a list of keys, mean those keys have current value
     *     1. increase, check if cur is head, if head, increase means new head with val + 1
     *     2. else, is in the middle, get it by map in O(1)
     *
     * 2. keep track of head and tail node, one is max, one is min, O(1)
     * 3. use map to access nodes by freq -> list<key> in O(1)
     * 4. decrease, get mid and check if val = 1? if so remove the key or entire node
     */
    class valueNode {
        valueNode preNode;
        valueNode nextNode;
